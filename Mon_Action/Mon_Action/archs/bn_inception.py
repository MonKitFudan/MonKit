...
pretrained_settings = {
    'bninception': {
        'imagenet': {
            'url': '/home1/xiaozf/.cache/torch/checkpoints/bn_inception-9f5701afb96c8044.pth',
            'input_space': 'BGR',
            'input_size': 224,
            'input_range': [0, 255],
            'mean': [104, 117, 128],
...
def bninception(pretrained='imagenet'):
    ...
    if pretrained is not None:
        ...
        # model.load_state_dict(model_zoo.load_url('https://www.dropbox.com/s/35ftw2t4mxxgjae/BNInceptionFlow-ef652051.pth.tar?dl=1'))
        model_BNInception = torch.load(settings['url'])
        ...
        model.load_state_dict(model_BNInception)
