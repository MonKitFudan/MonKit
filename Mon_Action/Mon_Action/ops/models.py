...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        if self.dropout == 0:
            ...
            print('prepare new fc:')
            print(feature_dim, num_class)
    ...
    def _prepare_base_model(self, base_model):
        ...
        if ('resnet' in base_model) or ('resnest' in base_model) or ('resnext' in base_model) or ('seres' in base_model):
            if 'resnet' in base_model:
                self.base_model = getattr(torchvision.models, base_model.split('p')[0])(True)
            elif 'resnest' in base_model:
                self.base_model = torch.hub.load('zhanghang1989/ResNeSt', base_model, pretrained=True)
            elif 'resnext' in base_model:
                self.base_model = resnext101_32x8d(pretrained=True)
            elif 'seres' in base_model:

                from model import SEResnet
                self.base_model = SEResnet.SENet(
                        block=SEResnet.SEResNetBottleneck,
                        layers=[3, 4, 23, 3],
                        groups=1,
                        reduction=16) #se_resnet101
                self.base_model.load_state_dict(torch.load('/home/fair/Desktop/xzf/tsm/se_resnet101-7e38fcc6.pth'))

                # self.base_model = senet.legacy_seresnet50(pretrained=True)
