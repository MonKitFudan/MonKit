...
class TSNDataSet(data.Dataset):
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            # center = np.random.randint(low=self.num_segments / 2,
            #                            high=record.num_frames - (self.num_segments / 2))
            # l = list()
            # for i in range(center - int(self.num_segments / 2), center):
            #     l.append(i)
            # for i in range(center, center + int(self.num_segments / 2)):
            #     l.append(i)
            # offsets = np.multiply(l, 1)
            return offsets + 1
