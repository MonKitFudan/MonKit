...
def eval_video(video_data, net, this_test_segments, modality):
    ...
    with torch.no_grad():
        ...
        f_rst = open('/home1/xiaozf/pycharmpro/tsm/rst_ucf101_train_split_1.txt','a+')
        f_rst.write(str(rst.cpu().numpy().tolist())+'--')
        # f_lbl = open('/home1/xiaozf/pycharmpro/tsm/lbl_ucf101_train_split_1.txt','a+')
        # f_lbl.write(str(label)+'\n')
        # f_inx = open('/home1/xiaozf/pycharmpro/tsm/inx_ucf101_train_split_1.txt','a+')
        # f_inx.write(str(index)+'\n')
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        this_rst_list = []
        this_label = None
        for n_seg, (_, (data, label,index)), net, modality in zip(test_segments_list, data_label_pairs, net_list, modality_list):
            rst = eval_video((i, data, label,index), net, n_seg, modality)
